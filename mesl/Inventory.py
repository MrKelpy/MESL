...
class Inventory():
    def __init__(self, window, versionLevel):
        size = window.size
    ...
    def hotbar(self, arg):
        if arg < 1 or arg > 9:
            raise_error(UnreachableHotbarSlot, f'The minecraft hotbar has 9 slots, numbered from 1-9. Therefore, {arg} could not be reached.')
        else:
            try:
                arg = str(arg)
                dictionary = {"1":HOTBAR_SLOT_1,
                              "2":HOTBAR_SLOT_2,
                              "3":HOTBAR_SLOT_3,
                              "4":HOTBAR_SLOT_4,
                              "5":HOTBAR_SLOT_5,
                              ...
                              "8":HOTBAR_SLOT_8,
                              "9":HOTBAR_SLOT_9}
                keyboard.press_and_release(dictionary[str(arg)])
    ...
    def drop(self, whole = False, amount : int = 1):
        if whole:
            ...
            if (amount > 1 and amount < 64):
                ...
                raise_error(InvalidDropAmount, f'Invalid drop amount. "{amount}" The minimal amount is 1 and the max is 64')
    ...
    def sendtohotbar(self, row, column, hotbar):
        self.slot(column, row)
        self.hotbar(hotbar)


    def pickplace(self, row, column, relative = False):
        self.slot(column, row)
        if relative:
